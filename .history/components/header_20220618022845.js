...
export default function Header(props) {
    ...
    >
        <AnimatePresence>
            ...
            >
                <Link href="/">
                    <div   className="flex items-end justify-end w-16 h-16 px-1 py-0 text-lg font-bold uppercase transition duration-500 bg-gray-200 shadow-sm dark:bg-black dark:text-white"
        ...
        <div className={styles.middle}>
            <ul className="flex items-center justify-around h-full text-xl uppercase transition duration-500 delay-300"
                // className={styles.links} 
            >
                {links.map((links, i) => (
                    <motion.div
                        key={links.id}
                        className={router.pathname === links.href ? "font-medium p-4 relative border-b-4" : "font-medium p-4 relative" }
        ...
        <div className={styles.right}>
            <div onClick={() => setTheme(colorTheme)}>
                    {colorTheme === "light" ?
                    (
                        <div   className="w-[50px] h-[50px] rounded-full flex justify-center items-center text-2xl bg-gray-200 cursor-pointer transition-all duration-300 ease hover:rotate-[360deg] active:scale-50 xl:mr-20 dark:bg-black dark:text-white"
