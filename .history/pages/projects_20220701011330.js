...
const About = () => {
  ...
  const currentTheme = theme === "system" ? systemTheme : theme;
    ...
    const container = {
      ...
      </svg>snap</div>, desc: <p>Snap tested my ability to create dropdown navigation menus, a common pattern on larger sites. It also provided some nice basic layout challenges.<br/><br/><span className="font-bold uppercase text-[1.25rem]">Technologies used</span><br /> <div className="flex flex-wrap w-full gap-4 mt-2 h-fit"><span className="p-1 text-white bg-red-500 rounded text-[1.2rem]">Figma</span><span className="p-1 text-white bg-blue-500 rounded text-[1.2rem]">Next.js</span><span className="p-1 text-white bg-violet-500 rounded text-[1.2rem]">React</span><span className="p-1 text-white bg-blue-400 rounded text-[1.2rem]">Tailwind CSS</span><span className="p-1 text-white bg- rounded text-[1.2rem]">Headless UI</span><span className="p-1 text-white bg-black rounded text-[1.2rem]">Framer Motion</span></div></p>, src: 'snap-projects-mobile-hero', srcdark: 'snap-projects-mobile-hero-dark', href: 'https://brimasnap.vercel.app/', hrefp: 'live site'},
