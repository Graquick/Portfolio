...
const About = () => {
  ...
  const currentTheme = theme === "system" ? systemTheme : theme;
    ...
    const container = {
      ...
      </svg>snap</div>, desc: <p>Snap tested my ability to create dropdown navigation menus, a common pattern on larger sites. It also provided some nice basic layout challenges.<br/><br/><span className="font-bold uppercase text-[1.25rem]">Technologies used</span><br /> <div className="flex flex-wrap w-full gap-4 mt-2 h-fit"><span className="p-1 text-black font-medium rounded text-[1.2rem] w-fit flex items-center justify-center gap-1">Figma <Icon icon="lucide:figma" /></span><span className=" text-black font-medium rounded w-fit flex items-center justify-center gap-1 text-[2rem]"><Icon icon="logos:nextjs" /></span><span className="p-1 text-black font-medium rounded-lg text-[1.2rem] w-fit flex items-center justify-center gap-1 border-2 border-black ">React<Icon icon="logos:react" /></span></div></p>, src: 'snap-projects-mobile-hero', srcdark: 'snap-projects-mobile-hero-dark', href: 'https://brimasnap.vercel.app/', hrefp: 'live site'},
